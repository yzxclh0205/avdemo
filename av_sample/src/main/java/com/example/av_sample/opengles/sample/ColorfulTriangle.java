 ...
 * 彩色三角形
 */
public class ColorfulTriangle extends Triangle {
    private static final String TAG = "ColorfulTriangle";

    // 支持顶点颜色的顶点着色器
    private final String vertexColorShaderCode =
    ...
    public static int createOpenGLProgram(String vertexSource, String fragmentSource) {
        int vertex = ShaderUtil.loadShader(GLES20.GL_VERTEX_SHADER, vertexSource);
        if (vertex == 0) {
            Log.e(TAG, "loadShader vertex failed");
            return 0;
        }
        int fragment = ShaderUtil.loadShader(GLES20.GL_FRAGMENT_SHADER, fragmentSource);
        if (fragment == 0) {
